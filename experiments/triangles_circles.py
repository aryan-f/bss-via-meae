...
class Experiment(pl.LightningModule):
    def __init__(self, config, x_plot=None):
        ...
        self.model = model_import(input_channels=config.input_channels, image_hw=config.image_hw,
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.zero_loss:
            z_zeros = [torch.zeros(x.shape[0], self.hidden // self.num_encoders, z[0].shape[-1], z[0].shape[-1]).to(
                self.device) for _ in range(self.num_encoders)]
