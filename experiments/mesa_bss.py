...
class Experiment(pl.LightningModule):
    def __init__(self, config, x_plot=None):
        ...
        self.model = model_import(input_channels=config.input_channels, input_length=config.input_length,
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.zero_loss:
            z_zeros = [torch.zeros(1, self.hidden // self.num_encoders, z[0].shape[-1]).to(self.device) for _ in
    ...
    def save_inference_samples(self, patient):
        ...
        colors = ['#24e254', '#12e29f', '#00e1e9', '#05b5f4', '#0a89ff', '#535def', '#9c31df', '#c91976', '#f5000c',
                  '#f5000c', '#f5000c',
                  '#f5000c']  ##https://coolors.co/24e254-12e29f-00e1e9-05b5f4-0a89ff-535def-9c31df-c91976-f5000c
        clip = 512
        line_width = 1.5
        idx = 1
        ...
        plt.plot(np.arange(len(x_cpu)), x_cpu, label=f'{self.input_signal_type} (Input)', c='#787878',
        ...
        for i, y_i in enumerate(y):
            plt.plot(np.arange(len(y_i[idx][clip:-clip])), z_norm(y_i[idx][clip:-clip]) - 8 * (i + 2),
                     label=f'Enc. {i} Source Pred.', alpha=0.9, c=colors[i + 1], linewidth=line_width)
    ...
    def configure_optimizers(self):
        ...
        scheduler = {
            'scheduler': torch.optim.lr_scheduler.StepLR(optimizer, step_size=self.lr_step_size, gamma=0.1,
                                                         last_epoch=-1),
